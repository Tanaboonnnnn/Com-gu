...
Headless Linux setup is self-contained in v3.3.1: run `comgu setup --name <machine-name>` and then `comgu doctor`. When no stronger credential source is available, setup creates a validated user-private `0600` profile key automatically; no manual `COMGU_CREDENTIAL_KEY` export is required.
