...
CLI ต้องใช้ **Node.js 22+**. แตกไฟล์แล้วรัน `comgu.cmd` บน Windows หรือ `./comgu` บน Linux; ใช้ `comgu setup` สำหรับตั้งค่า profile ครั้งแรก และ `comgu status` เพื่อตรวจ runtime ที่กำลังถือ profile อยู่
