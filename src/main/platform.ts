...
export function hostPlatformInfo(platform: NodeJS.Platform = process.platform): PlatformInfo {
  ...
  if (platform === 'linux') return { family: 'linux', name: 'Linux', desktopAutomation: true };
  return { family: 'other', name: platform, desktopAutomation: false };
}

/**
 * Desktop control is available on Windows and Linux; macOS remains Core-only in V1. Keep stored choices intact
